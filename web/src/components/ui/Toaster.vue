    ...
    >
      <TransitionGroup name="toast">
        <div
          ...
          :key="toast.id"
          class="glass pointer-events-auto relative flex items-start gap-2.5 overflow-hidden rounded-xl py-3 pl-4 pr-3 shadow-xl shadow-black/35"
          role="status"
        >
          <span
            class="absolute inset-y-0 left-0 w-0.5"
            :class="barColors[toast.variant]"
            aria-hidden="true"
          />
          <component
            :is="icons[toast.variant]"
            class="mt-0.5 h-4 w-4 shrink-0"
            :class="iconColors[toast.variant]"
          />
          <div class="min-w-0 flex-1">
            <p class="text-sm font-medium text-fg">{{ toast.title }}</p>
            <p v-if="toast.message" class="mt-0.5 break-words text-xs text-fg-secondary">
              {{ toast.message }}
            </p>
          </div>
          <button
